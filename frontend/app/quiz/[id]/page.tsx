...
export default function QuizPage() {
    ...
    const interval = setInterval(() => {
      setParticipants((prev) =>
        prev
          .map((p) => {
            if (p.isYou) {
              return { ...p, score, streak }
            }
            // More realistic score updates based on question difficulty
            const baseIncrease = Math.floor(Math.random() * 100) + 50
            const timeBonus = Math.floor(Math.random() * 30)
            return {
              ...p,
              score: p.score + baseIncrease + timeBonus,
              streak: Math.random() > 0.3 ? p.streak + 1 : Math.max(0, p.streak - 1),
            }
          })
  ...
  const usePowerUp = useCallback(
    ...
    [powerUps, isAnswered, nextQuestion, setTimeLeft, setPowerUps],
  )
